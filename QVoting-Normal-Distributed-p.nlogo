...
referenda-own [outcome referendum-number]
...
to vote [active-referendum]
  ...
  ask voters [
    ...
    (ifelse votes = 0  ; JACOB: See the documentation on ifelse. If you wrap ifelse in parentheses, you can use it like if, elif, elif... in python. Do that to flatten out these nested conditionals so they aren't nested
      ...
      item [referendum-number] of active-referendum utilities > 0
      [
...
to change-referenda-position [active-referendum]
  ask referenda [set ycor 40]  ;; JACOB: Code like these 2 lines should either be commented, or wrapped in a procedure with a descriptive name. I prefer the latter to keep the bigger procedure shorter.
...
to-report voice-credits-spent [active-referendum]
  ...
  [min list ( (item [referendum-number] of active-referendum utilities * average-marginal-pivotality) ^ 2) voice-credits ]  ;; JACOB: Makes sense, but somethine like this should be commented
  [(item [referendum-number] of active-referendum utilities * average-marginal-pivotality) ^ 2]
...
to-report payoff ;;should be zero when setup
  ...
  let payoff-sum 0
  ask voters [
    set payoff-sum payoff-sum + item [referendum-number] of last-referendum utilities
  ]
...
to-report sum-of-utilities
  ...
  let utility-sum 0
  ask voters [
    set utility-sum utility-sum + item [referendum-number] of last-referendum utilities
  ]
  report utility-sum
end
...
"" ""
PENS
"default" 1.0 1 -13840069 true "" "histogram filter [a -> a >= 0][item [referendum-number] of last-referendum utilities] of voters"
"pen-1" 1.0 1 -2674135 true "" "histogram filter [a -> a < 0][item [referendum-number] of last-referendum utilities] of voters"
...
219
251
variance-of-marginal-pivotality
variance-of-marginal-pivotality
0
1
0.1

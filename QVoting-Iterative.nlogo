...
to-report voice-credits-spent [active-referendum] ; voter function
  let spent-voice-credits 0

  (
    ifelse (ticks = 0) or (last-voting-method = "1p1v")
    [set spent-voice-credits 1]
    ((voice-credits >= abs(last-votes-for - last-votes-against)) and ((last-outcome and utilities < 0) or (not last-outcome and utilities > 0)))
    [set spent-voice-credits abs(last-votes-for - last-votes-against) + 1]
    ; else
    [set spent-voice-credits 0]
  )
  set last-voice-credits-spent spent-voice-credits
  set voice-credits voice-credits - spent-voice-credits + voice-credits-per-tick
...
to spawn-voters-with-utilities
  ask n-of number-of-voters patches with [pycor < 35][
    sprout-voters 1 [
      ...
      set color white
      set voice-credits voice-credits-per-tick  ;; JACOB: This should maybe be more than what is given on each tick subsequently
      set shape "cow"
  ]]
end

to spawn-referenda
  ask patches with [pycor = 40 and pxcor = 20][
    sprout-referenda 1 [
      set shape "square"
      set color white
      set size 2
...
setup</setup>
  ...
  <experiment name="1p1v-vs-QV-no" repetitions="1" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>

...
to vote
  set list-of-votes (list)
  repeat count referenda [set list-of-votes fput 0 list-of-votes]

  ask voters [
      let referenda-index 0
      repeat count referenda [
        set list-of-votes replace-item referenda-index list-of-votes (item referenda-index list-of-votes + item referenda-index utilities)
        set referenda-index referenda-index + 1
      ]
  ]
end
...
to check-results
  ask referenda [
    set outcome item referendum-number list-of-votes > 0
    ifelse outcome
    [set color green]
    [set color red]
  ]
...
to spawn-voters-with-utilities
  ask n-of number-of-voters patches with [pycor < 35][
    sprout-voters 1 [
      ...
      set utilities (list)
      repeat number-of-issues [set utilities fput ((random-float 2) - 1) utilities]
      let normalize sqrt sum map [u -> u ^ 2] utilities
      set utilities map [u -> u / normalize] utilities
      set color white
      set shape "person"
  ]]
end
